			...
			switch ( $value['type'] ) {
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $i . '" value="'.$select_ID.'" ' . selected(isset($smof_data[$value['id']]), $select_ID, false) . ' />'.$option.'</option>';	 
						$i++;
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'heading':
					...
					$menu .= '<li class="'. $header_class .'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"><i class='. $value['icon'].'></i>'.  $value['name'] .'</a></li>';
